...
```{r ex2_part2}
gap_ca <- gap %>% filter(country=="Canada")
ggplot(data=gap_ca, mapping=aes(x=gdpPercap, y=lifeExp)) + geom_point(color="red") + geom_line(color="black")
```
...
```{r ex2_part3}
gap_af <- gap %>% filter(continent=="Africa")
ggplot(data=gap_af, mapping=aes(x=gdpPercap, y=lifeExp)) + geom_point(color="purple")
```
...
```{r ex3_part1}
gap_EuAf <- gap %>% filter(continent=="Europe" | continent== "Africa", year==1992)

ggplot(data=gap_EuAf, mapping=aes(x=gdpPercap, y=lifeExp, color=continent)) + geom_point()
```
...
ggplot(data=gap_2007, mapping=aes(x=continent, y=gdpPercap)) + geom_boxplot(fill="darkgreen")
```
